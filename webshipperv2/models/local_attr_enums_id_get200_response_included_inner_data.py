...
LOCALATTRENUMSIDGET200RESPONSEINCLUDEDINNERDATA_ONE_OF_SCHEMAS = ["LocalAttrs"]

class LocalAttrEnumsIdGet200ResponseIncludedInnerData(BaseModel):
    """
    LocalAttrEnumsIdGet200ResponseIncludedInnerData
    """
    # data type: LocalAttrs
    oneof_schema_1_validator: Optional[LocalAttrs] = None
    actual_instance: Optional[Union[LocalAttrs]] = None
    one_of_schemas: List[str] = Literal["LocalAttrs"]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = LocalAttrEnumsIdGet200ResponseIncludedInnerData.model_construct()
        error_messages = []
        match = 0
        # validate data type: LocalAttrs
        if not isinstance(v, LocalAttrs):
            error_messages.append(f"Error! Input type `{type(v)}` is not `LocalAttrs`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in LocalAttrEnumsIdGet200ResponseIncludedInnerData with oneOf schemas: LocalAttrs. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in LocalAttrEnumsIdGet200ResponseIncludedInnerData with oneOf schemas: LocalAttrs. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into LocalAttrs
        try:
            instance.actual_instance = LocalAttrs.from_json(json_str)
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into LocalAttrEnumsIdGet200ResponseIncludedInnerData with oneOf schemas: LocalAttrs. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into LocalAttrEnumsIdGet200ResponseIncludedInnerData with oneOf schemas: LocalAttrs. Details: " + ", ".join(error_messages))
        else:
            return instance
