...
ACTIONSIDGET200RESPONSEINCLUDEDINNERDATA_ONE_OF_SCHEMAS = ["Automations"]

class ActionsIdGet200ResponseIncludedInnerData(BaseModel):
    """
    ActionsIdGet200ResponseIncludedInnerData
    """
    # data type: Automations
    oneof_schema_1_validator: Optional[Automations] = None
    actual_instance: Optional[Union[Automations]] = None
    one_of_schemas: List[str] = Literal["Automations"]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ActionsIdGet200ResponseIncludedInnerData.model_construct()
        error_messages = []
        match = 0
        # validate data type: Automations
        if not isinstance(v, Automations):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Automations`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in ActionsIdGet200ResponseIncludedInnerData with oneOf schemas: Automations. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in ActionsIdGet200ResponseIncludedInnerData with oneOf schemas: Automations. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into Automations
        try:
            instance.actual_instance = Automations.from_json(json_str)
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into ActionsIdGet200ResponseIncludedInnerData with oneOf schemas: Automations. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ActionsIdGet200ResponseIncludedInnerData with oneOf schemas: Automations. Details: " + ", ".join(error_messages))
        else:
            return instance
