...
CSVRULESIDGET200RESPONSEINCLUDEDINNERDATA_ONE_OF_SCHEMAS = ["CsvMappings"]

class CsvRulesIdGet200ResponseIncludedInnerData(BaseModel):
    """
    CsvRulesIdGet200ResponseIncludedInnerData
    """
    # data type: CsvMappings
    oneof_schema_1_validator: Optional[CsvMappings] = None
    actual_instance: Optional[Union[CsvMappings]] = None
    one_of_schemas: List[str] = Literal["CsvMappings"]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = CsvRulesIdGet200ResponseIncludedInnerData.model_construct()
        error_messages = []
        match = 0
        # validate data type: CsvMappings
        if not isinstance(v, CsvMappings):
            error_messages.append(f"Error! Input type `{type(v)}` is not `CsvMappings`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in CsvRulesIdGet200ResponseIncludedInnerData with oneOf schemas: CsvMappings. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in CsvRulesIdGet200ResponseIncludedInnerData with oneOf schemas: CsvMappings. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into CsvMappings
        try:
            instance.actual_instance = CsvMappings.from_json(json_str)
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into CsvRulesIdGet200ResponseIncludedInnerData with oneOf schemas: CsvMappings. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into CsvRulesIdGet200ResponseIncludedInnerData with oneOf schemas: CsvMappings. Details: " + ", ".join(error_messages))
        else:
            return instance
