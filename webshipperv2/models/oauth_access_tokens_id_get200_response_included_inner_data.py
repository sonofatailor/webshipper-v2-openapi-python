...
OAUTHACCESSTOKENSIDGET200RESPONSEINCLUDEDINNERDATA_ONE_OF_SCHEMAS = ["OauthApplications"]

class OauthAccessTokensIdGet200ResponseIncludedInnerData(BaseModel):
    """
    OauthAccessTokensIdGet200ResponseIncludedInnerData
    """
    # data type: OauthApplications
    oneof_schema_1_validator: Optional[OauthApplications] = None
    actual_instance: Optional[Union[OauthApplications]] = None
    one_of_schemas: List[str] = Literal["OauthApplications"]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = OauthAccessTokensIdGet200ResponseIncludedInnerData.model_construct()
        error_messages = []
        match = 0
        # validate data type: OauthApplications
        if not isinstance(v, OauthApplications):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OauthApplications`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in OauthAccessTokensIdGet200ResponseIncludedInnerData with oneOf schemas: OauthApplications. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in OauthAccessTokensIdGet200ResponseIncludedInnerData with oneOf schemas: OauthApplications. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into OauthApplications
        try:
            instance.actual_instance = OauthApplications.from_json(json_str)
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into OauthAccessTokensIdGet200ResponseIncludedInnerData with oneOf schemas: OauthApplications. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into OauthAccessTokensIdGet200ResponseIncludedInnerData with oneOf schemas: OauthApplications. Details: " + ", ".join(error_messages))
        else:
            return instance
