...
WEBHOOKFAILURESIDGET200RESPONSEINCLUDEDINNERDATA_ONE_OF_SCHEMAS = ["Webhooks"]

class WebhookFailuresIdGet200ResponseIncludedInnerData(BaseModel):
    """
    WebhookFailuresIdGet200ResponseIncludedInnerData
    """
    # data type: Webhooks
    oneof_schema_1_validator: Optional[Webhooks] = None
    actual_instance: Optional[Union[Webhooks]] = None
    one_of_schemas: List[str] = Literal["Webhooks"]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = WebhookFailuresIdGet200ResponseIncludedInnerData.model_construct()
        error_messages = []
        match = 0
        # validate data type: Webhooks
        if not isinstance(v, Webhooks):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Webhooks`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in WebhookFailuresIdGet200ResponseIncludedInnerData with oneOf schemas: Webhooks. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in WebhookFailuresIdGet200ResponseIncludedInnerData with oneOf schemas: Webhooks. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into Webhooks
        try:
            instance.actual_instance = Webhooks.from_json(json_str)
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into WebhookFailuresIdGet200ResponseIncludedInnerData with oneOf schemas: Webhooks. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into WebhookFailuresIdGet200ResponseIncludedInnerData with oneOf schemas: Webhooks. Details: " + ", ".join(error_messages))
        else:
            return instance
