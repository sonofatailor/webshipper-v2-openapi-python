...
MAILTEMPLATESIDGET200RESPONSEINCLUDEDINNERDATA_ONE_OF_SCHEMAS = ["Brands"]

class MailTemplatesIdGet200ResponseIncludedInnerData(BaseModel):
    """
    MailTemplatesIdGet200ResponseIncludedInnerData
    """
    # data type: Brands
    oneof_schema_1_validator: Optional[Brands] = None
    actual_instance: Optional[Union[Brands]] = None
    one_of_schemas: List[str] = Literal["Brands"]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = MailTemplatesIdGet200ResponseIncludedInnerData.model_construct()
        error_messages = []
        match = 0
        # validate data type: Brands
        if not isinstance(v, Brands):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Brands`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in MailTemplatesIdGet200ResponseIncludedInnerData with oneOf schemas: Brands. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in MailTemplatesIdGet200ResponseIncludedInnerData with oneOf schemas: Brands. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into Brands
        try:
            instance.actual_instance = Brands.from_json(json_str)
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into MailTemplatesIdGet200ResponseIncludedInnerData with oneOf schemas: Brands. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into MailTemplatesIdGet200ResponseIncludedInnerData with oneOf schemas: Brands. Details: " + ", ".join(error_messages))
        else:
            return instance
