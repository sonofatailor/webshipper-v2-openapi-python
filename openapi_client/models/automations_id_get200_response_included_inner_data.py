...
AUTOMATIONSIDGET200RESPONSEINCLUDEDINNERDATA_ONE_OF_SCHEMAS = ["ActivityTypes"]

class AutomationsIdGet200ResponseIncludedInnerData(BaseModel):
    """
    AutomationsIdGet200ResponseIncludedInnerData
    """
    # data type: ActivityTypes
    oneof_schema_1_validator: Optional[ActivityTypes] = None
    actual_instance: Optional[Union[ActivityTypes]] = None
    one_of_schemas: List[str] = Literal["ActivityTypes"]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = AutomationsIdGet200ResponseIncludedInnerData.model_construct()
        error_messages = []
        match = 0
        # validate data type: ActivityTypes
        if not isinstance(v, ActivityTypes):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ActivityTypes`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in AutomationsIdGet200ResponseIncludedInnerData with oneOf schemas: ActivityTypes. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in AutomationsIdGet200ResponseIncludedInnerData with oneOf schemas: ActivityTypes. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into ActivityTypes
        try:
            instance.actual_instance = ActivityTypes.from_json(json_str)
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into AutomationsIdGet200ResponseIncludedInnerData with oneOf schemas: ActivityTypes. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into AutomationsIdGet200ResponseIncludedInnerData with oneOf schemas: ActivityTypes. Details: " + ", ".join(error_messages))
        else:
            return instance
