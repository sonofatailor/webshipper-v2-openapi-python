...
CARRIERSIDGET200RESPONSEINCLUDEDINNERDATA_ONE_OF_SCHEMAS = ["CarrierTypes", "ShippingAddresses"]

class CarriersIdGet200ResponseIncludedInnerData(BaseModel):
    """
    CarriersIdGet200ResponseIncludedInnerData
    """
    # data type: CarrierTypes
    oneof_schema_1_validator: Optional[CarrierTypes] = None
    # data type: ShippingAddresses
    oneof_schema_2_validator: Optional[ShippingAddresses] = None
    actual_instance: Optional[Union[CarrierTypes, ShippingAddresses]] = None
    one_of_schemas: List[str] = Literal["CarrierTypes", "ShippingAddresses"]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = CarriersIdGet200ResponseIncludedInnerData.model_construct()
        error_messages = []
        match = 0
        # validate data type: CarrierTypes
        if not isinstance(v, CarrierTypes):
            error_messages.append(f"Error! Input type `{type(v)}` is not `CarrierTypes`")
        else:
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in CarriersIdGet200ResponseIncludedInnerData with oneOf schemas: CarrierTypes, ShippingAddresses. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in CarriersIdGet200ResponseIncludedInnerData with oneOf schemas: CarrierTypes, ShippingAddresses. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into CarrierTypes
        try:
            instance.actual_instance = CarrierTypes.from_json(json_str)
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into CarriersIdGet200ResponseIncludedInnerData with oneOf schemas: CarrierTypes, ShippingAddresses. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into CarriersIdGet200ResponseIncludedInnerData with oneOf schemas: CarrierTypes, ShippingAddresses. Details: " + ", ".join(error_messages))
        else:
            return instance
