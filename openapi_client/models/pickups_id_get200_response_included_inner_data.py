...
PICKUPSIDGET200RESPONSEINCLUDEDINNERDATA_ONE_OF_SCHEMAS = ["Carriers", "ShippingAddresses"]

class PickupsIdGet200ResponseIncludedInnerData(BaseModel):
    """
    ...
    actual_instance: Optional[Union[Carriers, ShippingAddresses]] = None
    one_of_schemas: List[str] = Literal["Carriers", "ShippingAddresses"]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = PickupsIdGet200ResponseIncludedInnerData.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in PickupsIdGet200ResponseIncludedInnerData with oneOf schemas: Carriers, ShippingAddresses. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in PickupsIdGet200ResponseIncludedInnerData with oneOf schemas: Carriers, ShippingAddresses. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into PickupsIdGet200ResponseIncludedInnerData with oneOf schemas: Carriers, ShippingAddresses. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PickupsIdGet200ResponseIncludedInnerData with oneOf schemas: Carriers, ShippingAddresses. Details: " + ", ".join(error_messages))
        else:
            return instance
